...
const C = {
  ...
  purple:"#a855f7", text:"#eeebe6", muted:"#6a6560",
};
const CAT_COLORS = ["#e8392a","#f07320","#eab308","#22c55e","#3b82f6","#a855f7"];
const LABELS = ["A","B","C","D"];
const DOC_TYPES = [
  { key:"source",     label:"Sources",       icon:"🔗", color:"#3b82f6" },
  { key:"fiche_memo", label:"Fiches mémo",   icon:"📋", color:"#a855f7" },
];
...
// ─── UI ─────────────────────────────────────────────────────────
const FL = ({children}) => (
  <label style={{display:"block",fontSize:11,fontWeight:600,color:C.muted,letterSpacing:1.8,textTransform:"uppercase",marginBottom:8,fontFamily:"'Barlow',sans-serif"}}>{children}</label>
);
...
const Btn = ({onClick,children,color=C.red,disabled,full,ghost,sm}) => (
  <button onClick={onClick} disabled={disabled} style={{
    ...
    cursor:disabled?"default":"pointer",fontWeight:600,fontSize:sm?12:14,
...
const Wrap = ({children}) => (
  <div style={{fontFamily:"'Barlow',sans-serif",background:C.bg,minHeight:"100vh",color:C.text}}>{children}</div>
);
  ...
  return (
    <div style={{maxWidth:680}}>
      <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:28}}>
        ...
        <h2 style={{fontFamily:"Oswald,sans-serif",fontSize:17,letterSpacing:2,textTransform:"uppercase",margin:0}}>{qs.some(x=>x.id===initial?.id)?"Éditer la question":"Nouvelle question"}</h2>
      </div>
      <ErrBox msg={saveErr}/>
        ...
        <div>
          ...
          {(q.opts||[]).map((opt,i)=>(
            <div key={i} style={{display:"flex",gap:10,marginBottom:10,alignItems:"center"}}>
              <button onClick={()=>setQ(x=>({...x,correct:i}))} style={{width:34,height:34,flexShrink:0,border:`2px solid ${q.correct===i?C.green:C.border}`,borderRadius:2,background:q.correct===i?"#0a2214":"transparent",color:q.correct===i?C.green:C.muted,fontFamily:"Oswald,sans-serif",fontWeight:700,fontSize:13,cursor:"pointer"}}>{LABELS[i]}</button>
              <input value={opt||""} onChange={e=>{const o=[...q.opts];o[i]=e.target.value;setQ(x=>({...x,opts:o}));}} placeholder={`Réponse ${LABELS[i]}`} style={{flex:1,background:C.surf,border:`1px solid ${q.correct===i?"#1a4a28":C.border}`,borderRadius:2,color:C.text,padding:"10px 13px",fontSize:14,fontFamily:"'Barlow',sans-serif",outline:"none"}}/>
            </div>
          ))}
        </div>
...
function CatForm({initial,cats,onSave,onCancel,saving,saveErr}) {
  ...
  return (
    <div style={{maxWidth:480}}>
      <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:28}}>
        <button onClick={onCancel} style={{background:"none",border:"none",color:C.muted,cursor:"pointer",fontSize:22,padding:0}}>←</button>
        <h2 style={{fontFamily:"Oswald,sans-serif",fontSize:17,letterSpacing:2,textTransform:"uppercase",margin:0}}>{cats.some(c=>c.id===initial?.id)?"Éditer la catégorie":"Nouvelle catégorie"}</h2>
      </div>
      <ErrBox msg={saveErr}/>
      <div style={{display:"flex",flexDirection:"column",gap:18}}>
        ...
        <div>
          ...
          <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
            {CAT_COLORS.map(col=>(
              <div key={col} onClick={()=>setCat(x=>({...x,color:col}))} style={{width:38,height:38,borderRadius:3,background:col,cursor:"pointer",border:cat.color===col?"3px solid #fff":"3px solid transparent",boxSizing:"border-box",transition:"all 0.12s"}}/>
            ))}
          </div>
        </div>
        <div style={{display:"flex",gap:12}}>
...
function DocForm({initial,cats,docs,onSave,onCancel,saving,saveErr}) {
  ...
  return (
    <div style={{maxWidth:580}}>
      <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:28}}>
        <button onClick={onCancel} style={{background:"none",border:"none",color:C.muted,cursor:"pointer",fontSize:22,padding:0}}>←</button>
        <h2 style={{fontFamily:"Oswald,sans-serif",fontSize:17,letterSpacing:2,textTransform:"uppercase",margin:0}}>{docs.some(x=>x.id===initial?.id)?"Éditer le document":"Nouveau document"}</h2>
      </div>
      <ErrBox msg={saveErr}/>
      <div style={{display:"flex",flexDirection:"column",gap:18}}>
        <div><FL>Catégorie *</FL>
          <select value={d.catId} onChange={e=>setD(x=>({...x,catId:e.target.value}))}
            style={{width:"100%",background:C.surf,border:`1px solid ${C.border}`,borderRadius:2,color:C.text,padding:"11px 13px",fontSize:14,fontFamily:"'Barlow',sans-serif",outline:"none"}}>
            {cats.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}
          </select>
        </div>
        <div><FL>Type de document *</FL>
          <div style={{display:"flex",gap:10}}>
            {DOC_TYPES.map(t=>(
              <button key={t.key} onClick={()=>setD(x=>({...x,type:t.key}))}
                style={{flex:1,background:d.type===t.key?t.color+"22":"transparent",color:d.type===t.key?t.color:C.muted,border:`1px solid ${d.type===t.key?t.color:C.border}`,borderRadius:2,padding:"10px 14px",cursor:"pointer",fontFamily:"'Barlow',sans-serif",fontWeight:600,fontSize:14,transition:"all 0.15s"}}>
                {t.icon} {t.label}
              </button>
            ))}
          </div>
        </div>
        <div><FL>Titre *</FL><Field value={d.title||""} onChange={e=>setD(x=>({...x,title:e.target.value}))} placeholder="Ex : Référentiel national secours à personnes"/></div>
        <div><FL>Description (optionnel)</FL><Field value={d.description||""} onChange={e=>setD(x=>({...x,description:e.target.value}))} placeholder="Courte description du document..." rows={2}/></div>
        <div>
          <FL>Fichier ou lien</FL>
          {/* Upload */}
          <div onClick={()=>fileRef.current.click()}
            style={{border:`2px dashed ${file?C.green:C.border}`,borderRadius:3,padding:"18px",textAlign:"center",cursor:"pointer",marginBottom:10,transition:"all 0.15s",background:file?"#081e10":"transparent"}}>
            <input ref={fileRef} type="file" accept=".pdf,.doc,.docx,.png,.jpg,.jpeg" onChange={handleFile} style={{display:"none"}}/>
            {file
              ? <span style={{color:C.green,fontSize:13}}>📄 {file.name}</span>
              : <span style={{color:C.muted,fontSize:13}}>📁 Cliquez pour uploader un fichier (PDF, image…)</span>
            }
          </div>
          {/* OR URL */}
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}>
            <div style={{flex:1,height:1,background:C.border}}/>
            <span style={{color:C.muted,fontSize:12}}>ou</span>
            <div style={{flex:1,height:1,background:C.border}}/>
          </div>
          <Field value={d.url||""} onChange={e=>{setD(x=>({...x,url:e.target.value}));setFile(null);}} placeholder="Coller un lien (Google Drive, site web…)"/>
        </div>
        <div style={{display:"flex",gap:12}}>
          <Btn onClick={()=>valid&&onSave(d,file,setUp)} disabled={!valid||saving||uploading} color={C.green}>
...
export default function App() {
  ...
  useEffect(()=>{
    ...
    const {data:{subscription}} = sb.auth.onAuthStateChange(async(event,session)=>{
      ...
      else if(session?.user&&(event==="SIGNED_IN"||event==="INITIAL_SESSION")) await onSession(session.user);
  ...
  const onSession = async(u)=>{
    ...
    await Promise.all([loadData(),loadMyResults(u.id),loadDocs()]);
    setPage("home");
  };
  ...
  const loadDocs = async()=>{
    ...
    if(data) setDocs(data.map(d=>({id:d.id,catId:d.cat_id,title:d.title,type:d.type,description:d.description,url:d.url})));
  };
  ...
  const startExam = (cat)=>{
    ...
    answersRef.current=[];
    setExamCat(cat);setExamList(list);setExamIdx(0);setPicked(null);setShown(false);setLog([]);
    setPage("exam");
  };
  const validate = ()=>{
        ...
        <div style={{background:C.card,border:`1px solid ${C.border}`,borderTop:`3px solid ${C.red}`,borderRadius:3,padding:"34px 30px",width:"100%",maxWidth:360}}>
          <div style={{display:"flex",background:C.surf,borderRadius:2,padding:3,marginBottom:26}}>
            {[["login","Connexion"],["register","Inscription"]].map(([m,l])=>(
              <button key={m} onClick={()=>{setAuthMode(m);setAErr("");}} style={{flex:1,background:authMode===m?C.red:"transparent",color:authMode===m?"#fff":C.muted,border:"none",borderRadius:1,padding:"9px",cursor:"pointer",fontFamily:"'Barlow',sans-serif",fontWeight:600,fontSize:13,transition:"all 0.15s"}}>{l}</button>
            ))}
          </div>
          <div style={{display:"flex",flexDirection:"column",gap:13}}>
            ...
            {aErr&&<div style={{color:aErr.startsWith("✓")?C.green:C.red,fontSize:13,lineHeight:1.5,background:aErr.startsWith("✓")?"#081e10":"#1e0808",border:`1px solid ${aErr.startsWith("✓")?"#1a4a28":"#4a1a10"}`,borderRadius:2,padding:"10px 12px"}}>{aErr}</div>}
      ...
      <Wrap>
        <Topbar
          ...
          right={<>
            ...
            <Btn onClick={()=>{setResCatFilter("all");setPage("resources");}} ghost sm>📚 Ressources</Btn>
  ...
  // ── RESOURCES ───────────────────────────────────────────────
  if(page==="resources"){
    const filteredDocs = resCatFilter==="all" ? docs : docs.filter(d=>d.catId===resCatFilter);
    ...
    return (
      <Wrap>
        <Topbar
          ...
          back={()=>setPage("home")}
          right={<span style={{color:C.muted,fontSize:13}}>👤 {pseudo}</span>}
        />

        {/* Filtre catégories */}
        <div style={{background:C.surf,borderBottom:`1px solid ${C.border}`,padding:"12px 24px",display:"flex",gap:8,flexWrap:"wrap",alignItems:"center"}}>
          <button onClick={()=>setResCatFilter("all")}
            style={{background:resCatFilter==="all"?"#fff":"transparent",color:resCatFilter==="all"?"#000":C.muted,border:`1px solid ${resCatFilter==="all"?"#fff":C.border}`,borderRadius:20,padding:"5px 16px",cursor:"pointer",fontSize:13,fontFamily:"'Barlow',sans-serif",fontWeight:600,transition:"all 0.15s"}}>
            Toutes
          </button>
          {cats.map(cat=>(
            <button key={cat.id} onClick={()=>setResCatFilter(cat.id)}
              style={{background:resCatFilter===cat.id?cat.color:"transparent",color:resCatFilter===cat.id?"#fff":C.muted,border:`1px solid ${resCatFilter===cat.id?cat.color:C.border}`,borderRadius:20,padding:"5px 16px",cursor:"pointer",fontSize:13,fontFamily:"'Barlow',sans-serif",fontWeight:600,transition:"all 0.15s"}}>
              {cat.name}
            </button>
          ))}
        </div>
        ...
        <div style={{maxWidth:900,margin:"0 auto",padding:"40px 24px"}}>
          {filteredDocs.length===0?(
            <div style={{textAlign:"center",color:C.muted,padding:"80px 0"}}>
              ...
              <div style={{fontSize:15,marginBottom:6}}>Aucun document disponible</div>
              <div style={{fontSize:13}}>{isAdmin?"Ajoutez des documents depuis le panneau admin.":"L'administrateur n'a pas encore ajouté de documents."}</div>
            </div>
          ):(
            <>
              {DOC_TYPES.map(type=>{
                const typeDocs = filteredDocs.filter(d=>d.type===type.key);
                if(!typeDocs.length) return null;
                return (
                  <div key={type.key} style={{marginBottom:44}}>
                    {/* Section header */}
                    <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:20}}>
                      <div style={{width:3,height:24,background:type.color,borderRadius:2}}/>
                      <span style={{fontFamily:"Oswald,sans-serif",fontSize:14,letterSpacing:3,textTransform:"uppercase",color:type.color}}>{type.icon} {type.label}</span>
                      <div style={{flex:1,height:1,background:C.border}}/>
                      <span style={{color:C.muted,fontSize:12}}>{typeDocs.length} document{typeDocs.length>1?"s":""}</span>
                    </div>
                    ...
                    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))",gap:14}}>
                      {typeDocs.map(doc=>{
                        ...
                        const isLink = doc.url&&!isPdf;
                        return (
                          <div key={doc.id} style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:3,padding:"18px 20px",display:"flex",flexDirection:"column",gap:10}}>
                            {/* Badge type + catégorie */}
                            <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                              <span style={{fontSize:11,color:type.color,background:type.color+"18",border:`1px solid ${type.color}44`,borderRadius:20,padding:"2px 10px",fontWeight:600}}>{type.icon} {type.label}</span>
                              {cat&&<span style={{fontSize:11,color:cat.color,background:cat.color+"18",border:`1px solid ${cat.color}44`,borderRadius:20,padding:"2px 10px",fontWeight:600}}>{cat.name}</span>}
                            </div>

                            {/* Titre */}
                            <div style={{fontFamily:"Oswald,sans-serif",fontSize:16,fontWeight:500,lineHeight:1.4}}>{doc.title}</div>

                            {/* Description */}
                            {doc.description&&<div style={{color:C.muted,fontSize:13,lineHeight:1.6}}>{doc.description}</div>}

                            {/* Bouton */}
                            {doc.url?(
                              <a href={doc.url} target="_blank" rel="noopener noreferrer"
                                style={{marginTop:"auto",display:"inline-flex",alignItems:"center",gap:8,background:type.color,color:"#fff",border:"none",borderRadius:2,padding:"9px 16px",cursor:"pointer",fontSize:13,fontWeight:600,fontFamily:"'Barlow',sans-serif",textDecoration:"none",transition:"opacity 0.15s",width:"fit-content"}}
                                onMouseEnter={e=>e.currentTarget.style.opacity="0.85"}
                                onMouseLeave={e=>e.currentTarget.style.opacity="1"}>
                                {isPdf?"📄 Ouvrir le PDF":"🔗 Ouvrir le lien"}
                              </a>
                            ):(
                              <span style={{color:C.muted,fontSize:12,fontStyle:"italic"}}>Aucun lien disponible</span>
                            )}
                          </div>
                        );
                      })}
    ...
    return (
      <Wrap>
        ...
        <div style={{maxWidth:660,margin:"0 auto",padding:"48px 24px"}}>
          ...
          {!shown?<Btn onClick={validate} disabled={picked===null} full>VALIDER</Btn>
      ...
      <Wrap>
        ...
        <div style={{background:C.surf,borderBottom:`1px solid ${C.border}`,display:"flex",padding:"0 24px",overflowX:"auto"}}>
          {[["qs","📋 Questions"],["cats","🗂 Catégories"],["documents","📚 Documents"],["classement","🏆 Classement"],["settings","⚙ Paramètres"]].map(([k,l])=>(
            <button key={k} onClick={()=>enterTab(k)} style={{background:"none",border:"none",borderBottom:`2px solid ${adminTab===k?C.red:"transparent"}`,color:adminTab===k?C.text:C.muted,padding:"13px 16px",cursor:"pointer",fontFamily:"'Barlow',sans-serif",fontWeight:500,fontSize:14,whiteSpace:"nowrap",transition:"all 0.15s"}}>{l}</button>
          ))}
        </div>
        ...
        <div style={{maxWidth:940,margin:"0 auto",padding:"34px 24px"}}>
          ...
          {adminTab==="documents"&&!editDoc&&(<>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:20}}>
              <h2 style={{fontFamily:"Oswald,sans-serif",fontSize:16,letterSpacing:2,textTransform:"uppercase",margin:0}}>Documents ({docs.length})</h2>
              <Btn onClick={()=>{setSaveErr("");setEditDoc({catId:cats[0]?.id||"",title:"",type:"source",description:"",url:""});}} disabled={cats.length===0}>+ Ajouter</Btn>
            </div>
            {cats.length===0&&<div style={{color:"#d97706",fontSize:13,background:"#1a1208",border:"1px solid #3a2810",borderRadius:2,padding:"10px 14px",marginBottom:14}}>⚠ Créez d'abord une catégorie.</div>}
            {docs.length===0?<div style={{textAlign:"center",color:C.muted,padding:"60px 0"}}>
              <div style={{fontSize:36,marginBottom:10}}>📂</div>
              Aucun document. Ajoutez des PDFs, liens ou fiches mémo.
            </div>:(
              <div style={{display:"flex",flexDirection:"column",gap:6}}>
                {docs.map(doc=>{
                  const cat=cats.find(c=>c.id===doc.catId);
                  ...
                  return (
                    <div key={doc.id} style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:2,padding:"12px 18px",display:"flex",alignItems:"center",justifyContent:"space-between",gap:16}}>
                      <div style={{flex:1,minWidth:0}}>
                        ...
                        {doc.url&&<div style={{fontSize:11,color:C.muted,marginTop:2,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>🔗 {doc.url}</div>}
                      </div>
                      <div style={{display:"flex",gap:8,flexShrink:0}}>
                        <button onClick={()=>{setSaveErr("");setEditDoc({...doc});}} style={{background:"transparent",border:`1px solid ${C.border}`,color:C.muted,borderRadius:2,padding:"4px 11px",cursor:"pointer",fontSize:12,fontFamily:"'Barlow',sans-serif"}}>✏ Éditer</button>
                        <button onClick={()=>setDelConfirm({type:"doc",id:doc.id})} style={{background:"transparent",border:"1px solid #381410",color:"#804040",borderRadius:2,padding:"4px 9px",cursor:"pointer",fontSize:12}}>🗑</button>
                      </div>
                    </div>
                  );
                })}
          ...
          <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.9)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:999,padding:24}}>
            <div style={{background:C.card,border:`1px solid ${C.border}`,borderTop:`3px solid ${C.red}`,borderRadius:3,padding:30,maxWidth:360,width:"100%"}}>
              ...
              <p style={{color:C.muted,fontSize:14,marginBottom:24,lineHeight:1.6}}>
                {delConfirm.type==="cat"?"⚠ Supprimer cette catégorie supprimera aussi toutes ses questions et documents."
                 :delConfirm.type==="doc"?"Ce document sera supprimé définitivement."
                 :"Cette action est irréversible."}
              </p>
              <div style={{display:"flex",gap:10}}>
                <Btn onClick={()=>{if(delConfirm.type==="q")deleteQ(delConfirm.id);else if(delConfirm.type==="doc")deleteDoc(delConfirm.id);else deleteCat(delConfirm.id);}} full>Supprimer</Btn>
